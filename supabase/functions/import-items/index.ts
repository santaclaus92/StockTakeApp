...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { sessionId, entity, pageSize = 1000, maxPages = 20 } = body ? JSON.parse(body) : {};

    // Paginate through Power Automate until all items are collected
    const all: any[] = [];
    let offset = 0;
    let page = 1;
    ...
    while (page <= maxPages) {
      const paResp = await fetch(PA_URL, {
        ...
        body: JSON.stringify({ sessionId, entity, limit: pageSize, offset, page }),
      });
      const raw = await paResp.text();
      if (!raw) break;

      const data = JSON.parse(raw);
      const arr: any[] = Array.isArray(data) ? data : (data.items || data.value || data.data || []);
      if (!arr.length) break;
    ...
    if (!all.length) throw new Error("No items returned from SAP.");

    const toInsert = all.map((i: any) => ({
    ...
    // Write to DB with service role key — bypasses RLS
    ...
    // Single delete before inserting all pages
    if (sessionId) {
    ...
    return new Response(JSON.stringify({ count: toInsert.length, pagesFetched: page - 1 }), {
