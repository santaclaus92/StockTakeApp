...
module.exports = async (req, res) => {
  ...
  // Vercel might pass it via query or we parse from URL
  const { id } = req.query;
    ...
    switch (req.method) {
      ...
      case 'POST': {
        ...
        if (!payload || typeof payload !== 'object') {
          return res.status(400).json({ message: 'POST requires a JSON object' });
        }
        ...
        const { data, error } = await supabase
          ...
          .select();
        if (error) throw error;
        return res.status(201).json(Array.isArray(payload) ? data : data[0]);
      }
      ...
      case 'DELETE': {
        ...
        const { error } = await supabase
          ...
          .eq('id', id);
        if (error) throw error;
        return res.status(200).json({ message: `Item ${id} removed` });
      }
