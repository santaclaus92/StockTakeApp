...
CREATE TABLE IF NOT EXISTS pairs (
  ...
  counter2_name    TEXT        DEFAULT NULL,
  counter2_absent  BOOLEAN     NOT NULL DEFAULT FALSE,
  progress         INTEGER     NOT NULL DEFAULT 0,
  created_at       TIMESTAMPTZ          DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS session_attendees (
  ...
  lunch_out   TIMESTAMPTZ DEFAULT NULL,
  lunch_in    TIMESTAMPTZ DEFAULT NULL,
  check_out   TIMESTAMPTZ DEFAULT NULL,
  PRIMARY KEY (session_id, user_id)
);
ALTER TABLE session_attendees ADD COLUMN IF NOT EXISTS check_in   TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE session_attendees ADD COLUMN IF NOT EXISTS lunch_out  TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE session_attendees ADD COLUMN IF NOT EXISTS lunch_in   TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE session_attendees ADD COLUMN IF NOT EXISTS check_out  TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE session_attendees DISABLE ROW LEVEL SECURITY;

-- Role-based access: 'Super Admin' manages admins + all admin pages; 'Admin' accesses sessions/admin pages; 'User' is count-only
...
-- Optional second counter in pair assignment
ALTER TABLE pairs ADD COLUMN IF NOT EXISTS counter2_name   TEXT    DEFAULT NULL;
ALTER TABLE pairs ADD COLUMN IF NOT EXISTS counter2_absent BOOLEAN NOT NULL DEFAULT FALSE;
...
-- Soft delete support (hard-purged by pg_cron after 3 months)
...
ALTER TABLE items ADD COLUMN IF NOT EXISTS admin_remark  TEXT;  -- Admin-only remarks (only editable by Admin role)
...
ALTER TABLE count_adjustments ADD COLUMN IF NOT EXISTS old_bin_location TEXT;
ALTER TABLE count_adjustments ADD COLUMN IF NOT EXISTS new_bin_location TEXT;
...
DECLARE
  ...
  pub_rows int;
BEGIN
  -- Strict security barrier: Only Super Admins can execute this function
  IF (auth.jwt() -> 'app_metadata' ->> 'role') IS DISTINCT FROM 'Super Admin' THEN
    RAISE EXCEPTION 'Access denied: Must be Super Admin to change roles.';
  END IF;
  ...
  -- 2. Keep the public.users table in sync for the admin UI
  UPDATE public.users
  SET role = new_role
  WHERE email = target_email;
  GET DIAGNOSTICS pub_rows = ROW_COUNT;

  RETURN 'auth_updated=' || auth_rows || ' pub_updated=' || pub_rows;
END;
$$;
-- Allow authenticated (logged-in) users to call this function.
-- The body still enforces Super Admin via JWT — this just lets PostgREST route the call.
GRANT EXECUTE ON FUNCTION set_user_role(text, text) TO authenticated;

-- ============================================================
-- Soft-delete purge job (pg_cron)
-- Runs daily at 02:00 UTC and hard-deletes sessions that were
-- soft-deleted more than 3 months ago. Cascade handles items,
-- pairs, session_attendees, item_audit, and count_adjustments.
...
SELECT cron.schedule(
  ...
  '0 2 * * *',
  'DELETE FROM sessions WHERE is_deleted = TRUE AND deleted_at < NOW() - INTERVAL ''3 months'''
);

-- Strict roles mode: counter scans, checker approves before counts are final
ALTER TABLE sessions ADD COLUMN IF NOT EXISTS strict_roles BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE items ADD COLUMN IF NOT EXISTS staged_count_qty  NUMERIC DEFAULT NULL;
ALTER TABLE items ADD COLUMN IF NOT EXISTS checker_status    TEXT    DEFAULT NULL;
-- checker_status: NULL = not applicable | 'Pending' = awaiting checker | 'Approved' = checker approved | 'Rejected' = checker rejected
